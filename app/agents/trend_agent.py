...
def _significant_change(history: list) -> bool:
...
async def trend_node(state: MedInsightState) -> MedInsightState:
    ...
    1. Identify tests from state["extracted_tests"]; fall back to all tests for patient.
    2. For each test: load history, compute stats, build TrendResult.
    3. Tests with < 2 data points get direction = "insufficient_data".
    """
    patient_id = state["patient_id"]
    extracted = state.get("extracted_tests", [])
    test_names = [t.get("test_name", "") for t in extracted if t.get("test_name")]
        ...
        async with AsyncSessionLocal() as session:
            ...
            for test_name in test_names:
                ...
                if len(history) < 2:
                    ...
                    # Still emit a result so the report agent can display the message
                    single_point = history[0] if history else None
                    dp = (
                        [{
                            "date": str(single_point.report_date),
                            "value": single_point.value,
                            "status": single_point.status or "unknown",
                            "report_id": str(single_point.report_id) if single_point.report_id else None,
                        }]
                        if single_point
                        else []
                    )
                    trend = TrendResult(
                        ...
                        trend_description=(
                            f"Only {len(history)} result found for {test_name}. "
                            "Upload more reports over time to track your trend."
                        ),
                        reference_low=ref_map.get(test_name, (None, None))[0],
                        reference_high=ref_map.get(test_name, (None, None))[1],
                    )
                    trend_results.append(trend.model_dump())
                    continue
                ...
                direction = _compute_direction(
                    first_val, last_val, ref_low, ref_high, pct_diff
                )
                ...
                trend_description = (
                    f"{test_name} is {direction} "
                    f"({sign}{pct_diff:.1f}% over {months} month{'s' if months != 1 else ''}, "
                    f"from {history[0].value} on {d0} to {last_val} on {d1})."
                )
                if velocity_concern:
                    trend_description += " Rate of change is notable."
                if threshold_crossed:
                    trend_description += " Value has crossed the reference boundary."
                if sig_change:
                    trend_description += " A significant shift (>20%) was detected between readings."
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            for test_name in test_names:
                ...
                trend_description = (
                    f"{test_name} is {direction} "
                    f"({'+' if pct_diff >= 0 else ''}{pct_diff:.1f}% over {months} month{'s' if months != 1 else ''})."
                )
                if velocity_concern:
                    trend_description += " Rate of change is notable."
                if threshold_crossed:
